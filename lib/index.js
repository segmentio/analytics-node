...
/**
 * Add a `message` of type `action` to the queue and check whether it should be
 * flushed.
 *
 * @param {String} action
 * @param {Object} message
...
Analytics.prototype.enqueue = function(action, message, fn){
  fn = fn || noop;
  message.action = action;
  if (!message.timestamp) message.timestamp = new Date();

  debug('%s: %o', action, message);
  this.queue.push({
    message: message,
    callback: fn
  });
