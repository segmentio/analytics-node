 ...
 *   @param  {String}  userId - id for logged in visitor (needs sessionId if not provided)
...
Client.prototype.identify = function (identify, context) {
    ...
    if (timestamp && !_.isDate(timestamp))
        throw new Error('[segmentio]#identify: timestamp ' +
 ...
 *   @option {Date}  timestamp  - the Date object representing when the track occurred
 *
...
Client.prototype.track = function (track) {
    ...
    if (timestamp && !_.isDate(timestamp))
        throw new Error('[segmentio]#track: timestamp ' +
                        'must be provided as a date.');
    var message = {
